...
\begin{itemize}
\item transition:   
\item output: when powered on, the device manager will call on the display system module to output the date and time. It will also be responsible for modifying the state of the prompt generation module when an activity is sensed to prompt the user. %After the user completes a prompt (i.e. in the prompt generation module), the state of the data storage module will change to store their data. 
\item exception: \wss{if appropriate} 
\end{itemize}
...
Moving Average Algorithm Module (Section \ref{}) %uses this module becuase the heartrate uses this module. 
...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\hline
accelerometer\_data & tuple & - & badMPUReadings, badInitAcceleration \\ %include some made up bed_errors.
heartrate\_data & $\mathbb{R}$ & - & badHRReadings, badInitHR, badSmoothingHR \\
pedo\_count & - & $\mathbb{R}$ & \\
heartrate & - & $\mathbb{R}$ & \\
prompt\_interrupt & - & bool & \\ %this will send a signal to the prompt generation module in order to tell it that an activity has been sensed. Then that module will take control (this should probably be an access routine).
...
Accel\_power: bool \\
HR\_sensor\_power: bool \\ %%is the accelerometer and HR sensor powered.
...
%Moving Average Algorithm Module

\section{MIS of Moving Average Algorithm Module} \label{mMAA} 
