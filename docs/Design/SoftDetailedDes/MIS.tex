...
\subsubsection{Exported Constants}
\begin{itemize}
\item Minimum battery threshold for voltage
\item Maximum battery limit for voltage
\item Low charge indicator for 10\% limit
\end{itemize}
...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\hline
bed\_getBatteryLev & Bat\_Raw & $\mathbb{Z}$ & BED\_ERR\_BAT\\
\hline
\end{tabular}
\end{center}
...
bed\_getBatteryLev: 
\begin{itemize}
\item Transition: Extracts the current battery level
\item Output: Integer percentage value of the battery level.
\item Exception:  BED\_ERR\_BAT = Z
\end{itemize}

\subsubsection{Local Functions}
\begin{itemize}
\item bed\_lowIndicator: boolean
\end{itemize}
\newpage
...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\hline
listDir & FS, const char*, uint8\_t & dirname: string & BED\_ERR\_SD: $\mathbb{Z}$ \\
createDir & FS, const char *& - & BED\_ERR\_SD: $\mathbb{Z}$ \\
removeDir & FS, const char * & - & BED\_ERR\_SD: $\mathbb{Z}$ \\
\hline
\end{tabular}
\end{center}
...
\begin{itemize}
\item Size of the field: First Name = 25
\item Size of the field: Last Name = 25
\item Size of the field: Gender = 25
\item Size of the field: Phone Number = 20
\item Size of the field: Email = 25
\item Size of the field: Address = 255
\item Size of the field: Device model number = 50
\end{itemize}
...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\hline
object.connect & - & Read\_Data: string, integer & BED\_ERR\_DB: $\mathbb{Z}$ \\
object.cursor & Write\_Data: string, integer & BED\_ERR\_DB: $\mathbb{Z}$ \\
object.execute & Write\_Data: string, integer & BED\_ERR\_DB: $\mathbb{Z}$ \\
object.commit & Write\_Data: string, integer & BED\_ERR\_DB: $\mathbb{Z}$ \\
object.close & Write\_Data: string, integer & BED\_ERR\_DB: $\mathbb{Z}$ \\
\hline
\end{tabular}
\end{center}
...
\subsubsection{Access Routine Semantics}
\begin{itemize}
\item object.connect(): Connects to the database.
\item object.cursor(): Establishes connection between the database and the query.
\item object.execute():
\item object.commit():
\item object.close(): Disconnects from the database.
\end{itemize}
...
\begin{itemize}
\item ACCEL\_SENSITIVITY: Z
\item GYRO\_SENSITIVITY: Z
\item Threshold: Z
\item MPU\_CALIBRATION: Z
\end{itemize}

\subsubsection{Exported Access Programs}
\begin{itemize}
\item bed\_mpu\_detect
\item bed\_hr\_detect
\end{itemize}
...
\subsubsection{Access Routine Semantics}
\begin{itemize}
\item bed\_mpu\_detect: Returns the current values of the accelerometer and gyroscope.
\item bed\_hr\_detect: Returns the current values of heartrate sensor.
\end{itemize}

\subsubsection{Local Functions}
\begin{itemize}
\item bed\_mpu\_setup
\item bed\_hr\_setup
\end{itemize}
...
\subsubsection{Exported Constants}
\begin{itemize}
\item ACTIVITY\_STEPS
\item ACTIVITY\_IDLE\_RESET
\item ACTIVITY\_IDLE\_WAIT
\end{itemize}
